...
function parse_location(location_sexp) {
  ...
  if (location_sexp.children[0].source.toLowerCase() == ":error"){
    ...
    srcloc.error = util.from_lisp_string(location_sexp.children[1].source);
  } else {
    ...
    if (srcloc.buffer_type == 'file') {
      srcloc.file = util.from_lisp_string(raw_buffer_sexp.children[1].source);
    } else if (srcloc.buffer_type == 'buffer') {
      srcloc.buffer_name = util.from_lisp_string(raw_buffer_sexp.children[1].source);
    } else if (srcloc.buffer_type == 'buffer-and-file') {
      srcloc.buffer_name = util.from_lisp_string(raw_buffer_sexp.children[1].source);
      srcloc.file = util.from_lisp_string(raw_buffer_sexp.children[2].source);
    } else if (srcloc.buffer_type == 'source-form') {
      srcloc.source_form = util.from_lisp_string(raw_buffer_sexp.children[1].source);
    } else if (srcloc.buffer_type == 'zip') {
      srcloc.zip_file = util.from_lisp_string(raw_buffer_sexp.children[1].source);
      srcloc.zip_entry = util.from_lisp_string(raw_buffer_sexp.children[1].source);
    }
...
Client.prototype.on_swank_message = function(msg) {
  ...
  if (cmd == ":return") {
    ...
    this.on_handlers.print_string(util.from_lisp_string(sexp.children[1].source));
    ...
    this.on_handlers.new_package(util.from_lisp_string(sexp.children[1].source));
  } else if (cmd == ":debug") {
...
Client.prototype.read_from_minibuffer_handler = function(sexp) {
  var thread = sexp.children[1].source,
      ...
      prompt = util.from_lisp_string(sexp.children[3].source),
...
Client.prototype.y_or_n_p_handler = function(sexp) {
  var thread = sexp.children[1].source,
      tag = sexp.children[2].source,
      question = util.from_lisp_string(sexp.children[3].source);
    ...
    .then(function (ast) {
      try {
        return ast.children[0].children.map(function(competion) {
          return util.from_lisp_string(competion.source);
        });
      } catch (e) {
        return [];
      }
...
Client.prototype.debug_setup_handler = function(sexp) {
  ...
  obj.title = util.from_lisp_string(sexp.children[3].children[0].source);
  obj.type = util.from_lisp_string(sexp.children[3].children[1].source);
  obj.restarts = [];
  sexp.children[4].children.forEach(function(restart_sexp) {
    obj.restarts.push({
      cmd: util.from_lisp_string(restart_sexp.children[0].source),
      description: util.from_lisp_string(restart_sexp.children[1].source)
    });
  });
  obj.stack_frames = [];
  sexp.children[5].children.forEach(function(frame_sexp){
    ...
    obj.stack_frames.push({
      ...
      description: util.from_lisp_string(frame_sexp.children[1].source),
    ...
    return this.rex(cmd, pkg, thread).then(function(sexp) {
      if (util.from_lisp_bool(sexp.children[0])) {
        frame_info.locals = sexp.children[0].children.map(function(local_sexp) {
          return {
            name:  util.from_lisp_string(local_sexp.children[1].source),
            id:    Number(local_sexp.children[3].source),
            value: util.from_lisp_string(local_sexp.children[5].source)
          };
        });
      } else {
        frame_info.locals = [];
      ...
      if (util.from_lisp_bool(sexp.children[1])) {
        frame_info.catch_tags = sexp.children[1].children.map(function(tag_sexp) {
          return util.from_lisp_string(tag_sexp.source);
        });
      } else {
...
Client.prototype.debug_return_from_frame = function(frame, value, thread, pkg='COMMON-LISP-USER') {
  ...
  return this.rex(cmd, pkg, thread).then(function(rawErrorMessage) {
    if (util.from_lisp_bool(rawErrorMessage)){
      throw new Error(util.from_lisp_string(rawErrorMessage.source));
    }
  });
}
...
Client.prototype.debug_disassemble_frame = function(frame, thread, pkg='COMMON-LISP-USER') {
  ...
  return this.rex(cmd, pkg, thread).then(function(sexp) {
    return util.from_lisp_string(sexp.source);
  });
}
...
Client.prototype.debug_eval_in_frame = function(frame, expr, thread, pkg='COMMON-LISP-USER') {
  ...
  return this.rex(cmd, pkg, thread).then(function(package) {
    ...
    return util.from_lisp_string(result.source);
  });
}
      ...
      for(var i = 0; i < ast.children.length; i++) {
        try {
          ...
          if (srcloc.buffer_type != 'error') {
            ...
            refs.push({
              label: util.from_lisp_string(child_ast.children[0].source),
              location: srcloc
            });
          }
        } catch (e) {
...
Client.prototype.parse_inspection = function(pkg, result) {
  ...
  for (var i = 0; i < result.children.length; i += 2) {
    ...
    if (key == ':title') {
      // string
      inspection.title = util.from_lisp_string(val.source);
    } else if (key == ':id') {
      // integer
  ...
  return inspection_promise.then(function (inspector) {
    inspection.content = inspection.raw_content.map(function(elt) {
      if (elt.type == 'string') {
        return util.from_lisp_string(elt.source);
      } else { //type == 'list'
        return [elt.children[0].source,
                util.from_lisp_string(elt.children[1].source),
