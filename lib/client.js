...
Client.prototype.on_swank_message = function(msg) {
  ...
  if (cmd == ":return") {
    ...
    this.on_handlers.print_string(read_string(sexp.children[1].source));
    ...
    this.on_handlers.new_package(read_string(sexp.children[1].source));
  } else if (cmd == ":debug") {
...
Client.prototype.autodoc = function(sexp_string, cursor_position, pkg) {
  ...
  try {
    ...
    for(var i = 0; i < forms.children.length; i++) {
      ...
      output_forms.push(to_lisp_string(form.source));
    ...
    .then(function (ast) {
      try {
        return ast.children[0].children.map(function(competion) {
          return read_string(competion.source);
        });
      } catch (e) {
        return [];
      }
...
Client.prototype.debug_setup_handler = function(sexp) {
  ...
  obj.title = read_string(sexp.children[3].children[0].source);
  obj.type = read_string(sexp.children[3].children[1].source);
  obj.restarts = [];
  sexp.children[4].children.forEach(function(restart_sexp) {
    obj.restarts.push({
      cmd: read_string(restart_sexp.children[0].source),
      description: read_string(restart_sexp.children[1].source)
    });
  });
  obj.stack_frames = [];
  sexp.children[5].children.forEach(function(frame_sexp){
    ...
    obj.stack_frames.push({
      ...
      description: read_string(frame_sexp.children[1].source),
    ...
    return this.rex(cmd, "COMMON-LISP-USER", thread).then(function(sexp) {
      if (sexp.children[0].type.toLowerCase() == 'list'){
        frame_info.locals = sexp.children[0].children.map(function(local_sexp) {
          return {
            name:  read_string(local_sexp.children[1].source),
      ...
      if (sexp.children[1].type.toLowerCase() == 'list') {
        frame_info.catch_tags = sexp.children[1].children.map(function(tag_sexp) {
          return read_string(tag_sexp.source);
        });
      } else {
...
Client.prototype.debug_return_from_frame = function(frame, value, thread) {
  ...
  return this.rex(cmd, "COMMON-LISP-USER", thread).then(function(errorMessage) {
    return read_string(errorMessage.source);
  });
}
...
Client.prototype.debug_frame_source = function(frame, thread) {
  ...
  return this.rex(cmd, "COMMON-LISP-USER", thread).then(function(location_sexp) {
    ...
    if(location_sexp.children[0].source.toLowerCase() == ":error"){
      ...
      location.error = read_string(location_sexp.children[1].source);
    } else {
      ...
      if (location.buffer_type == 'file') {
        location.file = read_string(raw_buffer_sexp.children[1].source);
      } else if (location.buffer_type == 'buffer') {
        location.buffer_name = read_string(raw_buffer_sexp.children[1].source);
      } else if (location.buffer_type == 'buffer-and-file') {
        location.buffer_name = read_string(raw_buffer_sexp.children[1].source);
        location.file = read_string(raw_buffer_sexp.children[2].source);
      } else if (location.buffer_type == 'source-form') {
        location.source_form = read_string(raw_buffer_sexp.children[1].source);
      } else if (location.buffer_type == 'zip') {
        location.zip_file = read_string(raw_buffer_sexp.children[1].source);
        location.zip_entry = read_string(raw_buffer_sexp.children[1].source);
      }
...
Client.prototype.debug_disassemble_frame = function(frame, thread) {
  ...
  return this.rex(cmd, "COMMON-LISP-USER", thread).then(function(sexp) {
    return read_string(sexp.source);
  });
}
...
Client.prototype.debug_eval_in_frame = function(frame, expr, thread) {
  ...
  return this.rex(cmd, "COMMON-LISP-USER", thread).then(function(result) {
    return read_string(result.source);
  });
}
      ...
      for(var i = 0; i < ast.children.length; i++) {
        try {
          ...
          if (ast.children[i].children[1].children[1].children[0].source.toLowerCase() == ":file") {
            filename = read_string(ast.children[i].children[1].children[1].children[1].source);
          } else if (ast.children[i].children[1].children[1].children[0].source.toLowerCase() == ":buffer-and-file") {
            filename = read_string(ast.children[i].children[1].children[1].children[2].source);
          }

          // Push an appropriate reference!
          refs.push({
            label: read_string(ast.children[i].children[0].source),
            filename: filename,
...
Client.prototype.compile_string = function(compile_string, filename, filename_full, position, line, column, package) {
  ...
  return this.rex(cmd, package, "T").then(this.on_compilation);
}
...
Client.prototype.on_compilation = function(result) {
